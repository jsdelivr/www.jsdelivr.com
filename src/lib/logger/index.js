...
class FileStream extends fs.WriteStream {
	...
	write (record) {
		try {
			if (record.err) {
				record = _.assign({}, record, {
					err: Object.assign({}, record.err, { stack: record.err.stack || record.err.toString() }),
				});
			}
...
module.exports = bunyan.createLogger({
	...
	streams: [
		...
		{ level: bunyan.TRACE, type: 'raw', stream: new FileStream(path.join(loggerConfig.path, `${os.hostname()}-${process.pid}.log`)) },
		{ level: bunyan.ERROR, type: 'raw', stream: new OpbeatStream() },
	],
